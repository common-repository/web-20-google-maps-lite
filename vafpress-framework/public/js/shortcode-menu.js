	...
	$(document).ready(function(){
		...
		jQuery.fn.scReset = function(){
			...
			if ($.fn.select2)
			{
				...
				vp_w2gm.init_fontawesome_chooser($(this).find('.vp-js-fontawesome'));
				vp_w2gm.init_select2($(this).find('.vp-js-select2'));
				vp_w2gm.init_sorter($(this).find('.vp-js-sorter'));
			}
			return $(this);
		};

		// init controls
		vp_w2gm.init_controls($('.vp-sc-main'));

		// shortcode image controls event bind
		vp_w2gm.custom_check_radio_event(".vp-sc-dialog", ".vp-checkimage .field .input label");
		vp_w2gm.custom_check_radio_event(".vp-sc-dialog", ".vp-radioimage .field .input label");
		...
		$('.vp-sc-element .vp-sc-element-heading a').bind('click.vp_w2gm_sc', function(e){
			e.preventDefault();
			...
			else
			{
				...
				$modal.find('.vp-sc-element-form').vp_w2gm_slideUp();

				if($form.exists())
				{
					$parent.addClass('active');
					$form.vp_w2gm_slideDown();
				}
				else
				{
					code = decodeEntities(code);
					$modal.trigger('vp_w2gm_insert_shortcode', code);
					$modal.trigger('reveal:close');
				}
			}
		});

		$('.vp-sc-insert').bind('click.vp_w2gm_sc_insert', function(e){
			e.preventDefault();
			var $modal  = $(this).parents('.vp-sc-dialog'),
			...
			$form.on('submit', function(e){vp_w2gm.tinyMCE_save(); e.preventDefault();});
			$form.submit();
		...
		$('.vp-sc-cancel').bind('click.vp_w2gm_sc_cancel', function(e){
			e.preventDefault();
			$('.vp-sc-element').removeClass('active');
