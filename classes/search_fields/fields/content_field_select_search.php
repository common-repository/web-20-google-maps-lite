...
class w2gm_content_field_select_search extends w2gm_content_field_search {
	...
	public function searchConfigure() {
		...
		if (w2gm_getValue($_POST, 'submit') && wp_verify_nonce($_POST['w2gm_configure_content_fields_nonce'], W2GM_PATH)) {
			...
			if ($validation->run()) {
				...
				if ($wpdb->update($wpdb->w2gm_content_fields, array('search_options' => serialize(array('search_input_mode' => $result['search_input_mode'], 'checkboxes_operator' => $result['checkboxes_operator']))), array('id' => $this->content_field->id), null, array('%d')))
	...
	public function renderSearch($random_id, $columns = 2, $defaults = array()) {
		...
		w2gm_renderTemplate('search_fields/fields/select_checkbox_radio_input.tpl.php', array('search_field' => $this, 'columns' => $columns, 'random_id' => $random_id));
	}
