...
<h2>
	<?php _e('Change expiration date', 'W2GM'); ?>
</h2>
	...
	(function($) {
		...
		$(function() {
			$("#expiration_date").datepicker({
				showOn: "both",
				buttonImage: "<?php echo W2GM_RESOURCES_URL?>images/calendar.png",
...
<form action="<?php echo admin_url('options.php?page=w2gm_changedate&listing_id=' . $listing->post->ID . '&changedate_action=changedate&referer=' . urlencode($referer)); ?>" method="POST">
	<p>
	...
	<?php if ($action == 'show'): ?>
	<input type="submit" value="<?php esc_attr_e('Save changes', 'W2GM'); ?>" class="button button-primary" id="submit" name="submit">
	&nbsp;&nbsp;&nbsp;
