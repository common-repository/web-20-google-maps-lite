<script>
	(function($) {
		...
		$(function() {
			$("#w2gm-field-input-<?php echo $content_field->id; ?>").datepicker({
				<?php if (function_exists('is_rtl') && is_rtl()): ?>isRTL: true,<?php endif; ?>
				showButtonPanel: true,
				dateFormat: '<?php echo $dateformat; ?>',
				onSelect: function(dateText) {
					var tmstmp_str;
					var sDate = $("#w2gm-field-input-<?php echo $content_field->id; ?>").datepicker("getDate");
					if (sDate) {
					...
					$("input[name=w2gm-field-input-<?php echo $content_field->id; ?>]").val(tmstmp_str);
				}
			});
			<?php
			if ($lang_code = w2gm_getDatePickerLangCode(get_locale())): ?>
			$("#w2gm-field-input-<?php echo $content_field->id; ?>").datepicker($.datepicker.regional[ "<?php echo $lang_code; ?>" ]);
			<?php endif; ?>
			
			<?php if ($content_field->value['date']): ?>
			$("#w2gm-field-input-<?php echo $content_field->id; ?>").datepicker('setDate', $.datepicker.parseDate('dd/mm/yy', '<?php echo date('d/m/Y', $content_field->value['date']); ?>'));
			<?php endif; ?>
			$("#reset_date_<?php echo $content_field->id; ?>").click(function() {
				$.datepicker._clearDate('#w2gm-field-input-<?php echo $content_field->id; ?>');
			})
		});
	})(jQuery);
</script>
<div class="w2gm-form-group w2gm-field w2gm-field-input-block w2gm-field-input-block-<?php echo $content_field->id; ?>">
	...
	<div class="w2gm-col-md-10">
		<div class="w2gm-row w2gm-form-group w2gm-clearfix">
			<div class="w2gm-col-md-4 w2gm-has-feedback">
				<input type="text" id="w2gm-field-input-<?php echo $content_field->id; ?>" class="w2gm-field-input-datetime w2gm-form-control" />
				<span class="w2gm-glyphicon w2gm-glyphicon-calendar w2gm-form-control-feedback"></span>
				<input type="hidden" name="w2gm-field-input-<?php echo $content_field->id; ?>" value="<?php echo esc_attr($content_field->value['date']); ?>"/>
			</div>
			<div class="w2gm-col-md-3">
				<input type="button" class="w2gm-btn w2gm-btn-primary" id="reset_date_<?php echo $content_field->id; ?>" value="<?php esc_attr_e('reset', 'W2GM')?>" />
			</div>
		</div>
		<?php if ($content_field->is_time): ?>
		<div class="w2gm-row">
			<label class="w2gm-col-md-1"><?php _e('Time:', 'W2GM'); ?></label>
			<div class="w2gm-col-md-3">
				<select name="w2gm-field-input-hour_<?php echo $content_field->id; ?>" class="w2gm-form-control">
			...
			<div class="w2gm-col-md-3">
				<select name="w2gm-field-input-minute_<?php echo $content_field->id; ?>" class="w2gm-form-control">
