		...
		<script>
			(function($) {
				...
				$(function() {
					...
					$('body').on('mousewheel', '#w2gm-slider-wrapper-<?php echo $random_id; ?> .bx-viewport', function(event, delta, deltaX, deltaY) {
						if (delta > 0) {
					...
					$('body').on('mouseover', '#w2gm-slider-<?php echo $random_id; ?> .slide', function(event) {
						changeSlide_<?php echo $random_id; ?>($(this));
					});

					// Just change slide on click, this is only for touchscreens
					$('body').on('click', '.w2gm-touch #w2gm-slider-<?php echo $random_id; ?> .slide', function(event) {
						event.preventDefault();
						changeSlide_<?php echo $random_id; ?>($(this));
					});
					...
					function changeSlide_<?php echo $random_id; ?>(slide) {
						...
						$("#w2gm-big-slide-wrapper-<?php echo $random_id; ?> .w2gm-big-slide").css('background-image', 'url('+slide.find("img").attr("src")+')');
